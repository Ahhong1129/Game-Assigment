...
void GameWindows::init() {
	...
	wndClass.hIcon = LoadIcon(GetModuleHandle(NULL), MAKEINTRESOURCE(IDR_IDI_ICON11));
	wndClass.hInstance = hInstance;	//	GetModuleHandle(NULL);
	wndClass.lpfnWndProc = WindowProcedure;
	wndClass.lpszClassName = "2D Car Racing";
	wndClass.style = CS_HREDRAW | CS_VREDRAW;

	RegisterClass(&wndClass);
	g_hWnd = CreateWindowEx(0, wndClass.lpszClassName, "2D Car Racing", WS_OVERLAPPEDWINDOW, 0, 100, 340, 527, NULL, NULL, hInstance, NULL);
